...
// handle all of the page events and dispatches them to a handler, if one exists
fdx.Pages.Kernel = function (event){
    ...
    if (fdx && fdx.Pages && pageName && fdx.Pages[pageName] && fdx.Pages[pageName][eventType]){
        fdx.Pages[pageName][eventType].call(that);
    }
...
fdx.Events = function () {
    $("div[data-fdx-jspage]").on(
        ...
        fdx.Pages.Kernel).on("pageinit", fdx.hideAddressBar);
    } ();


// this is the handler for all page events
fdx.Pages.calculator = function(){
    var pageshow = function () {
            fdx.Display.init($("#displayControl")[0]);
            $("button").tap(function(event){
                var key = $(this).attr("data-fdx-tag"),
                    id = this.id;
                ...
                switch(id){
                    ...
                    case "keyDecimalPoint":
                        fdx.Display.enterDigit(key);
                        break;
                    case "keyC":
                        fdx.Display.clearDisplay();
                        break;
                    case "keyCe":
                        fdx.Display.clearError();
                        break;
                    case "keyAdd":
                        fdx.Display.setOperator("+");
                        break;
                    case "keySubtract":
                        fdx.Display.setOperator("-");
                        break;
